...
function setupTestEnvironment() {
  ...
  // Additional test-suite specific setup can be added here
}

function utilsIsCIEnvironment(): boolean {
  return utilIsCIEnvironment()
}

function utilsIsTestEnvironment(): boolean {
  return utilIsTestEnvironment()
}

type DatabaseProvider = 'memory' | 'sqlite' | 'd1'
type Runtime = 'node' | 'bun' | 'cloudflare-workers'
