 ...
 */
export async function clearOutbox<T extends Hono<any>>(app: T, jarKey = 'default'): Promise<void> {
...
export async function assertEmailSent<T extends Hono<any>>(
  ...
  if (!email) {
    ...
    const recipients = allEmails.flatMap((e) => e.to)
...
export function extractOTPCode(email: TestEmail): string | null {
  ...
  const patterns = [/\b(\d{4,8})\b/g, /code[:\s]*(\d{4,8})/i, /otp[:\s]*(\d{4,8})/i]
