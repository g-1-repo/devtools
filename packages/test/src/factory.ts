...
export class TestDataFactory {
  ...
  organization = this.createFactory((config?: FactoryConfig) => {
    ...
    return {
      ...
      website: faker.internet.url(),
      logo: faker.image.url({ width: 400, height: 300 }),
  ...
  order = this.createFactory((config?: FactoryConfig) => {
    ...
    return {
      ...
      trackingNumber: faker.datatype.boolean(0.7)
        ? `TRK${faker.number.int({ min: 1000000000, max: 9999999999 })}`
        : null,
  ...
  event = this.createFactory((config?: FactoryConfig) => {
    ...
    return {
      ...
      price: faker.datatype.boolean(0.7)
  ...
  apiKey = this.createFactory((config?: FactoryConfig) => ({
    ...
    permissions: faker.helpers.arrayElements(['read', 'write', 'delete', 'admin'], {
      min: 1,
      max: 3,
    }),
