  ...
  return [
    ...
    // Version Approval
    {
      ...
      task: async (ctx, helpers) => {
        ...
        helpers.setOutput(
          `Calculated version: ${ctx.version!.current} → ${ctx.version!.next} (${ctx.version!.type})`
        )
        ...
        if (approval === 'change') {
          ...
          const newNextVersion = semver.inc(
            ctx.version!.current,
            newVersionType as 'major' | 'minor' | 'patch'
          )
          if (!newNextVersion) {
            throw new Error(`Failed to calculate next version from ${ctx.version!.current}`)
          }
          ...
          helpers.setOutput(
            `Updated version: ${ctx.version!.current} → ${ctx.version!.next} (${ctx.version!.type})`
          )
        }

        helpers.setTitle(
          `Version approval - ✅ ${ctx.version!.current} → ${ctx.version!.next} (${ctx.version!.type})`
        )
      },
    },
