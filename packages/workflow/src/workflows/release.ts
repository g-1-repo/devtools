                ...
                for (const packageJsonPath of packageJsonFiles) {
                  ...
                  try {
                    ...
                    if (packageJson.name && packageJson.version) {
                      ...
                      helpers.setOutput(`Updated ${packageJson.name}: ${oldVersion} → ${ctx.version!.next}`)
                      updatedPackages++
                    }
                  } catch (error) {
                    helpers.setOutput(`Warning: Could not update ${packageJsonPath}: ${error instanceof Error ? error.message : String(error)}`)
                  }
                }
                
                helpers.setOutput(`✅ Synchronized versions across ${updatedPackages} packages in monorepo`)
              } catch (error) {
                helpers.setOutput(`Warning: Monorepo version synchronization failed: ${error instanceof Error ? error.message : String(error)}`)
