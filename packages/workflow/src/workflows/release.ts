    ...
    // Version Approval
    {
      ...
      task: async (ctx, helpers) => {
        ...
        if (options.nonInteractive || options.type) {
          helpers.setTitle(`Version approval - ✅ Using ${ctx.version!.type} bump (${ctx.version!.next})`)
          return
        }

        helpers.setOutput(`Calculated version: ${ctx.version!.current} → ${ctx.version!.next} (${ctx.version!.type})`)

        const approval = await select({
          message: `Approve version ${ctx.version!.next}?`,
          options: [
        ...
        if (approval === 'change') {
          ...
          // Recalculate version with new type
          const newNextVersion = semver.inc(ctx.version!.current, newVersionType as 'major' | 'minor' | 'patch')
          if (!newNextVersion) {
            throw new Error(`Failed to calculate next version from ${ctx.version!.current}`)
          }

          ctx.version!.next = newNextVersion
          ctx.version!.type = newVersionType as 'major' | 'minor' | 'patch'

          helpers.setOutput(`Updated version: ${ctx.version!.current} → ${ctx.version!.next} (${ctx.version!.type})`)
        }

        helpers.setTitle(`Version approval - ✅ ${ctx.version!.current} → ${ctx.version!.next} (${ctx.version!.type})`)
      },
    },
