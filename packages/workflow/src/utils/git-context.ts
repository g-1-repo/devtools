 ...
 */
export async function analyzeGitContext(workingDir: string = process.cwd()): Promise<GitContextInfo> {
  ...
  const isValidContext = !gitStatus.hasNestedGitRepo && (gitStatus.hasGitRepo || gitStatus.isInMonorepo)
 ...
 */
export async function findOptimalWorkingDirectory(startDir: string = process.cwd()): Promise<string> {
...
export async function createContextAwareGitOperations(workingDir?: string) {
  ...
  const optimalDir = workingDir ? await findOptimalWorkingDirectory(workingDir) : await findOptimalWorkingDirectory()
...
export async function validateWorkflowContext(workingDir: string = process.cwd()): Promise<{
  ...
  if (gitStatus.hasNestedGitRepo) {
    ...
    recommendations.push(`Remove nested .git directory and use monorepo at ${gitStatus.parentGitRoot}`)
  }
  ...
  if (context.isMonorepo && context.relativePath && !gitStatus.hasNestedGitRepo) {
    recommendations.push(`Consider running workflow commands from monorepo root: ${context.gitRoot}`)
  }
