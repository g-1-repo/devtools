...
export function createStyledBox(
  ...
  const bottomBorder = boxStyle.corner[2] + boxStyle.border.repeat(maxWidth - 2) + boxStyle.corner[3]
  ...
  console.log(`${chalk.gray('│')} ${boxStyle.title(titleText.padEnd(maxWidth - 4))} ${chalk.gray('│')}`)
  
  if (lines.length > 0) {
    console.log(`${chalk.gray('│')} ${' '.repeat(maxWidth - 4)} ${chalk.gray('│')}`)
    
    lines.forEach(line => {
      console.log(`${chalk.gray('│')} ${boxStyle.content(line.padEnd(maxWidth - 4))} ${chalk.gray('│')}`)
    })
  }
...
export function createSummaryTable(
  ...
  items.forEach(item => {
    ...
    const statusIcon = item.status ? 
      (item.status === 'success' ? G1_ICONS.success :
       item.status === 'error' ? G1_ICONS.error :
       item.status === 'warning' ? G1_ICONS.warning :
       G1_ICONS.info) : ''
    
    const statusColor = item.status ?
      (item.status === 'success' ? chalk.green :
